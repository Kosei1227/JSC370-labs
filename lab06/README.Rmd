...
stringr::str_extract(counts, "\\d[\\d,]*")
```
...
publications <- GET(
  ...
  query = list(
    db = "pubmed",
    id = I(id_string),  # I() prevents automatic encoding issues
    retmax = 1000,
    rettype = "abstract"
  )
)
...
cat(substr(publications_txt, 1, 1000))
```
...
institution <- str_extract_all(
  publications_txt,
  "\\b(University of [A-Za-z ]+|[A-Za-z ]+ Institute of [A-Za-z ]+)\\b"
) 
...
schools_and_deps <- str_extract_all(
  publications_txt,
  "\\b(School of [A-Za-z ]+|Department of [A-Za-z ]+)\\b"
)
...
# Extract individual articles from the XML response
pub_char_list <- xml2::xml_children(publications)

# Convert each XML element into a character string
pub_char_list <- sapply(pub_char_list, as.character)
...
num_missing_abstracts <- sum(is.na(abstracts))

# Print result
cat("Number of articles without abstracts:", num_missing_abstracts, "\n")
```
...
num_missing_titles <- sum(is.na(titles))

# Print result
cat("Number of articles without titles:", num_missing_titles, "\n")
```
